def training(mdl, X, y, folds, cv=2, metric="accuracy"):
    ...
    if cv == 1:
        ...
        results = None
    if cv == 2:
        training = sk_ms.cross_val_score(
            mdl, X, y, cv=folds, scoring=metric, n_jobs=-1)
...
def shap_df_one_record(data):
    ...
    top = [['LoS_student_prediction',
            float("NaN"),
            x['outValue']],
           ['LoS_base_prediction',
            float("NaN"),
            x['baseValue']],
           ['student_base_diff_val',
            float("NaN"),
    ...
    result = (
        pd.DataFrame(
            ...
            columns=[
                'characteristic',
                'student_data',
                'shap_val_log_odds']) .assign(
            shap_probability=lambda x: np.exp(
                x.shap_val_log_odds) /
            (
                1 +
                np.exp(
                    x.shap_val_log_odds))) .round(2))
